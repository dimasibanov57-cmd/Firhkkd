...
function victory() {
    ...
    setTimeout(() => {
        ...
        if (nextCh <= 100) {
            ctrl.innerHTML = `
                <button class="g" onclick="nextBoss()" style="grid-column:span 2;">⚔️ Следующий босс</button>
                <button class="b" onclick="showChapterSelect()" style="grid-column:span 2;">📖 Выбрать босса</button>
                <button class="back" onclick="backToMenu()" style="grid-column:span 2;">🔙 В меню</button>
            `;
        } else {
            ctrl.innerHTML = `
                <button class="gold" onclick="backToMenu()" style="grid-column:span 2;">🏆 Все боссы побеждены!</button>
            `;
        }
    }, 800);
}
...
function updateUI() {
    ...
    let progress = (game.p.xp / game.p.xn) * 100;
    if (el('menuProgress')) el('menuProgress').style.width = Math.min(100, progress) + '%';
}
...
function claimDaily() {
    ...
    updateDailyUI();
    updateUI();
    addLog('🎁 Ежедневный бонус: +' + reward + '🪙');
    alert('🎁 Получено ' + reward + ' монет!');
}

function save() {
    try {
        localStorage.setItem('gd', JSON.stringify({ 
            p: game.p, 
            ch: game.ch, 
            daily: game.daily 
        }));
